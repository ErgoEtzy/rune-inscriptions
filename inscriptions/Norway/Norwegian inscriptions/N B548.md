---
type: rune-inscription
inscription_id: "N B548"
found_location: "Gullskoen, Bryggen, Bergen"
parish: "Bergen"
municipality: "Bergen"
district: ""
dating: "M ca 1300"
rune_type: ""
carver: ""
material_type: "wood"
material: "trä"
style: ""
source: "Rundata-net / Scandinavian Runic-text Database"
tags:
  - runes
  - inscription
---

# N B548

## Core data

| Field | Value |
|---|---|
| Inscription ID | N B548 |
| Found location | Gullskoen, Bryggen, Bergen |
| Parish | Bergen |
| Municipality | Bergen |
| District |  |
| Current location | BRM 0/71491 |
| Original site |  |
| Dating period | M ca 1300 |
| Rune type |  |
| Carver |  |
| Number of crosses | 0 |
| Cross form |  |
| Object information | fyrkantig pinne |
| Material type | wood |
| Material | trä |
| Style |  |
| Oldest known coordinates | 60.398, 5.32 |
| Current coordinates |  |

## Transliteration

```text
§A ÷ o^l ber·ek · ypisæliu · a^rmg(l)(a)- §B sek færþ a^þ^u(s)u ærþa ygi(r) - §C : uil^ldae^k ÷ grimnis ÷ gil^ldi : (f)(a)^r(u)n^ ^n- §D n^u ha^ua ska^ld af stælda^r (s)-
```

## Normalisation to Old Scandinavian

```text
§A Ôl ber'k yppi-selju arm-glóðar(?), §B sé'k ferð "Ásu(?) ærða, "Yggjar ... §C Vilda'k "Grímnis gildi grun ... §D Nú hafa skald af stældar ...
```

## Normalization to Old West Norse

```text
§A Ôl ber'k yppi-selju arm-glóðar(?), §B sé'k ferð "Ásu(?) ærða, "Yggjar ... §C Vilda'k "Grímnis gildi grun ... §D Nú hafa skald af stældar ...
```

## Translation to English

[Norska] Jeg bærer fram et dikt (= Yggs/Odins øl) til selja som løfter armgloen (= kvinnen), - jeg ser Åses(?) ferd vokse - (Yggs) ... Jeg ville diktet (= drikke(laget) til Grimner/Odin) ... Nå har skaldene ?? stålsatt (= forsynt et dikt med innskutt setning) ...

## Translation to Swedish

Jeg bærer fram et dikt (= Yggs/Odins øl) til selja som løfter armgloen (= kvinnen), - jeg ser Åses(?) ferd vokse - (Yggs) ... Jeg ville diktet (= drikke(laget) til Grimner/Odin) ... Nå har skaldene ?? stålsatt (= forsynt et dikt med innskutt setning) ...

## Other information



## References

Swedish Runic Bibliography: https://riksarkivet.x-ref.se/DocList?find=RUNSIG%20%22N%20B548%22

## Images / links



## Interpretation notes

### Established evidence

### Likely / plausible interpretation

### Symbolic reading

### Speculative or uncertain

## Connections

- [[Rundata inscription index]]
- [[Scandinavian Runic-text Database]]
- [[Rundata-net]]
